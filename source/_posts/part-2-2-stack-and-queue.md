...
> **图示描述**：
> 一个垂直的、底部封闭的容器代表栈。
> 1.  **Push(10)**: 10 被放入容器底部。
> 2.  **Push(20)**: 20 被放在 10 的上面。
> 3.  **Push(30)**: 30 被放在 20 的上面。此时，30 在栈顶。
> 4.  **Pop()**: 30 从顶部被取出。此时，20 成为新的栈顶。
> 5.  **Peek()**: 查看栈顶，得到 20，但 20 仍然在栈中。
...
>
> **图示描述**：
> 一个水平的通道代表队列，左边是队首(Front)，右边是队尾(Rear)。
> 1.  **Enqueue(10)**: 10 从右侧进入通道。 `[10]`
> 2.  **Enqueue(20)**: 20 从右侧进入，排在 10 后面。 `[10, 20]`
> 3.  **Enqueue(30)**: 30 从右侧进入，排在 20 后面。 `[10, 20, 30]`
> 4.  **Dequeue()**: 10 从左侧离开。 `[20, 30]`
> 5.  **Peek()**: 查看队首，得到 20，但 20 仍然在队列中。
