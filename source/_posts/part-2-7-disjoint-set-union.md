...
> value: [0, 0, 0, 3, 3, 5]
> ```
> `parent[0]=0`, `parent[3]=3`, `parent[5]=5` 表示 0, 3, 5 是三个集合的根节点。
...
**步骤**：
1.  通过 `find(i)` 找到 `i` 的老大 `root_i`。
2.  通过 `find(j)` 找到 `j` 的老大 `root_j`。
3.  如果 `root_i` 和 `root_j` 不相同，说明他们不在一个集合，就将一个老大认另一个为父节点，例如 `parent[root_i] = root_j`。
...
**场景**：`find(4)`，初始结构为 `4 -> 3 -> 2 -> 1 -> 0`。

1.  **查找前**: `parent = [0, 0, 1, 2, 3]`
2.  **查找过程**: 从 4 开始，经过 3, 2, 1, 找到根 0。
3.  **压缩路径**: 在回溯时，将 4, 3, 2, 1 的父节点全部指向 0。
4.  **查找后**: `parent = [0, 0, 0, 0, 0]`
...
**实现**：
*   **按秩 (Rank)**：用一个 `rank` 数组记录每棵树的高度。合并时，将高度较小的树的根指向高度较大的树的根。
*   **按大小 (Size)**：用一个 `size` 数组记录每个集合的元素数量。合并时，将元素数量较少的集合合并到较多的集合上。
